...
class SubRangeSelect(DisplayCanvas):
    ...
    def indexify(self, val):
        """convert val to index in discrete ranges"""
        try:
            return self.steps.index(val)
        except ValueError:
            return -1
    ...
    def format_val(self, val, min=False, max=False):
        ...
        if self.CONTINUOUS:
            ...
            if min:
                return self.steps[int(math.ceil(val))]
            elif max:
    ...
    def get_selection(self):
        """return the limits of the selected subrange"""
        if self.CONTINUOUS:
            return self.subrange_min, self.subrange_max
        else:
            return (int(math.ceil(self.subrange_min)),
                    int(math.floor(self.subrange_max)))
